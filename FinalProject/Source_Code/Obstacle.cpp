...
void Obstacles::mapToBitMap(int bitmap[60][60],bool erase)
{
	...
	for (int i = 0;i<fabs(w); i+=1)
	{
		for (int j = 0; j < fabs(h); j += 1)
		{
			...
			{
				bitmap[(int)(x + ((float)w / fabs(w)) * i)][(int)(y + ((float)h / fabs(h)) * j)] = 0;
			}
		}
...
void Obstacles::setObstacle(int bitmap[60][60], bool erase)
{
	int lb, mb, rb, mx, my;
	FsGetMouseEvent(lb, mb, rb, mx, my);
